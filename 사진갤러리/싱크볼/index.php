...
$galleryData = getGalleryData('sink');
?>
<!DOCTYPE html>
<html lang="ko-KR">
<head>
    ...
    <title>아콴테 싱크볼 갤러리 | <?php echo e($siteSettings['site_name']); ?></title>
    <meta name="description" content="러블리키친 아콴테 싱크볼 시공 사진 갤러리">
    ...
    <style>
        .lightbox{display:none;position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,.95);z-index:10000;justify-content:center;align-items:center;opacity:0;transition:opacity .3s ease}
        .lightbox.active{display:flex;opacity:1}
        .lightbox-content{max-width:90%;max-height:90%;position:relative}
        .lightbox-content img{max-width:100%;max-height:90vh;object-fit:contain;border-radius:8px;box-shadow:0 20px 60px rgba(0,0,0,.5)}
        .lightbox-close{position:absolute;top:20px;right:20px;width:50px;height:50px;background:rgba(255,255,255,.1);border:none;border-radius:50%;color:#fff;font-size:28px;cursor:pointer;transition:all .3s ease;display:flex;align-items:center;justify-content:center}
        .lightbox-close:hover{background:rgba(255,255,255,.2);transform:rotate(90deg)}
        .lightbox-nav{position:absolute;top:50%;transform:translateY(-50%);width:50px;height:50px;background:rgba(255,255,255,.1);border:none;border-radius:50%;color:#fff;font-size:20px;cursor:pointer;transition:all .3s ease}
        .lightbox-nav:hover{background:rgba(255,255,255,.2)}
        .lightbox-prev{left:20px}.lightbox-next{right:20px}
        .gallery-page-item{cursor:pointer;transition:transform .3s ease}
...
<body>
    ...
    <main>
        <section class="page-hero"><div class="page-hero-bg"></div><div class="page-hero-overlay"></div><div class="container"><div class="page-hero-content" data-aos="fade-up"><nav class="breadcrumb"><a href="<?php echo SITE_URL; ?>/">홈</a><span><i class="fas fa-chevron-right"></i></span><a href="<?php echo SITE_URL; ?>/사진갤러리/">사진 갤러리</a><span><i class="fas fa-chevron-right"></i></span><span class="current">아콴테 싱크볼</span></nav><h1 class="page-title">아콴테 싱크볼 갤러리</h1><p class="page-subtitle">아콴테 싱크볼 시공 사진을 확인해보세요</p></div></div></section>
        ...
        <section class="content-section">
            <div class="container">
                <div class="section-header" data-aos="fade-up">
                    ...
                    <p class="section-desc">아콴테 싱크볼과 함께하는 깨끗한 변화</p>
                </div>
                ...
                <div class="gallery-page-grid" data-aos="fade-up">
                    ...
                    <p style="text-align:center; color:#999; grid-column:1/-1;">등록된 이미지가 없습니다.</p>
                    <?php endif; ?>
                </div>
    ...
    <div class="lightbox" id="lightbox">
        ...
        <div class="lightbox-content"><img id="lightbox-img" src="" alt="갤러리 이미지"></div>
    ...
    <script>
        const galleryItems=document.querySelectorAll('.gallery-page-item'),lightbox=document.getElementById('lightbox'),lightboxImg=document.getElementById('lightbox-img');let currentIndex=0,images=[];
        galleryItems.forEach((item,index)=>{const img=item.querySelector('img');if(img){images.push(img.src);item.addEventListener('click',()=>openLightbox(index))}});
        function openLightbox(index){currentIndex=index;lightboxImg.src=images[currentIndex];lightbox.classList.add('active');document.body.style.overflow='hidden'}
        function closeLightbox(){lightbox.classList.remove('active');document.body.style.overflow=''}
        function navigateLightbox(direction){currentIndex+=direction;if(currentIndex<0)currentIndex=images.length-1;if(currentIndex>=images.length)currentIndex=0;lightboxImg.src=images[currentIndex]}
